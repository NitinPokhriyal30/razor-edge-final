...
const OperationSingle = () => {
  ...
  useEffect(() => {
    const get = async () => {
      const res = await Axios.get("https://razor-edge-app.herokuapp.com/operation-jobs/" + id); 
      setData(res.data);
  ...
  const updateData = async () => {
    try {
      await Axios.put(`https://razor-edge-app.herokuapp.com/operation-jobs/${data._id}`, {
        title,
        location,
      });
  ...
  const deleteData = () => {
    Axios.delete(`https://razor-edge-app.herokuapp.com/operation-jobs/${data._id}`); 
    navigate("/admin-career");
  };
