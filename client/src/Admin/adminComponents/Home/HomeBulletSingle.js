...
const HomeBulletSingle = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      const res = await Axios.get("https://razor-edge-app.herokuapp.com/homebullet/" + id); 
      setData(res.data);
  ...
  const updateData = async () => {
    try {
      await Axios.put(`https://razor-edge-app.herokuapp.com/homebullet/${data._id}`, {
        text,
      });
  ...
  const deleteData = () => {
    Axios.delete(`https://razor-edge-app.herokuapp.com/homebullet/${data._id}`).then((res) =>
      navigate("/admin-dashboard")
    );
  };
